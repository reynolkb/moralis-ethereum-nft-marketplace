...
function NFTTokenIds({ inputValue, setInputValue }) {
	...
	const getMarketItem = (nft) => {
		const result = fetchMarketItems?.find((e) => e.nftContract === nft?.token_address && e.tokenId === nft?.token_id && e.sold === false && e.confirmed === true);
		return result;
	};
		...
		<>
			<div>
				{contractABIJson.noContractDeployed && (
					<>
						<Alert message='No Smart Contract Details Provided. Please deploy smart contract and provide address + ABI in the MoralisDappProvider.js file' type='error' />
				...
				{inputValue !== 'explore' && totalNFTs !== undefined && (
					<>
						{!fetchSuccess && (
							<>
								<Alert message='Unable to fetch all NFT metadata... We are searching for a solution, please try again later!' type='warning' />
								<div style={{ marginBottom: '10px' }}></div>
							</>
						)}
						<div style={styles.banner}>
							<Image preview={false} src={NFTTokenIds[0]?.image || 'error'} fallback={fallbackImg} alt='' style={styles.logo} />
					...
					{inputValue === 'explore' &&
						NFTCollections?.map((nft, index) => (
							<Card
								...
								cover={<Image preview={false} src={nft?.image || 'error'} fallback={fallbackImg} alt='' style={{ height: '240px' }} />}
								key={index}>
								<Meta title={nft.name} />
							</Card>
						))}
				...
				{getMarketItem(nftToBuy) ? (
					<Modal title={`Buy ${nftToBuy?.name} #${nftToBuy?.token_id}`} visible={visible} onCancel={() => setVisibility(false)} onOk={() => purchase()} okText='Buy'>
					...
					<Modal title={`Buy ${nftToBuy?.name} #${nftToBuy?.token_id}`} visible={visible} onCancel={() => setVisibility(false)} onOk={() => setVisibility(false)}>
						<img
							src={nftToBuy?.image}
							style={{
								width: '250px',
